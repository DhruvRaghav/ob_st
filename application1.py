...
def process_brand_image(option, file):
    if file is not None:
        img = io.imread(file)
        ...
        if option in ['brand_box']:
            ...
            result = run(source=f'API_DATA/{option}', nosave=True, box=True, device='cpu')
            blur.rectblur_NEW(f'API_DATA/{option}/test.png', result, option)
            result_image_path = f'C:/dataset/object_detection-main/API_DATA/{option}/test.png'
...
def process_object_image(option, file):
    if file is not None:
        img = io.imread(file)
        # Save the uploaded image
        save_path = f'API_DATA/{option}/test.png'
...
def main():
    ...
    if st.button("Process"):
        if brand_option == "Select an option" and general_option == "Select an option":
            ...
            process_brand_image(brand_option.lower().replace(" ", "_"), file)

        elif general_option != "Select an option":
            process_object_image(general_option.lower().replace(" ", "_"), file)
